...
3. Compose `$FEATURE_FOLDER/final-readiness-report.md` with these sections:
   ...
   - **Residual MAJOR items** — total count + per-gate breakdown of MAJOR findings open when a gate passed under the relaxed rule (iterations 3 and up, `blockers=0`; majors do not block at that tier, dispositioned or not). Report, per gate, whether the relaxed tier's single final fixer batch ran and whether it was re-reviewed: that majors-only final fix is authorized to close the gate WITHOUT a further reviewer round, so some residual majors may be unreviewed-after-fix — state it rather than claiming re-review. Read from each gate's summary file (the summarizer records deferred majors there). Present this section only when at least one gate carried deferred majors. NOTE: this section's presence is NOT the trigger for `READY_WITH_NOTES` — a relaxed-tier pass forces `READY_WITH_NOTES` on its own (see the readiness-verdict rule), so a clean relaxed pass produces `READY_WITH_NOTES` with this section absent.
