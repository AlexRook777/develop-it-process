...
4. Apply the iteration-dependent gate (see "Review-gate severity policy") against the catalog counts from step 3 — **iterations 1–2:** run this step when `blockers + majors > 0`, and always loop back to step 1 for a full re-review afterwards; **iterations 3 and up:** `blockers = 0` alone passes the gate, so run this step only when `blockers > 0` (full fix→re-review loop, as in iterations 1–2) or when majors remain and no final batch has run yet at this tier — in that one case dispatch the final batch below and then go straight to step 5 (gate passed), WITHOUT looping back to step 1:
   ...
   - **Iterations 1–2 only:** increment N and loop from step 1 — the reviewers ALWAYS run again against the fixer's new revision in the strict tier. **Iterations 3 and up:** loop back to step 1 only if this round's trigger was a BLOCKER; if the trigger was majors-only, this was the single authorized unreviewed final fix — record every still-open major (unbatched, or left `blocked` by the fixer) in `followups.jsonl` via `append_followup`, then go to step 5 and pass the gate.
5. When the gate passes — `blockers=0` and (iterations 1–2: `majors=0`) or (iterations 3+: `blockers=0` alone, after the single final fixer batch has run if majors were present):
...
If iteration cap (`review_iteration_cap`) trips with any active reviewer still reporting an open BLOCKER, HALT and surface to user with residual findings paths and the spec path (`event=ITERATION_CAP_REACHED`). A cap reached with `blockers=0` is NOT a HALT however many majors remain — it passes per the relaxed gate (and sets the readiness verdict to `READY_WITH_NOTES`), with every residual major recorded in the gate summary and `followups.jsonl`.
...
One JSON object per line, each with: <!-- INCLUDE-BEGIN: finding-record-schema reviewer_role=spec-reviewer-claude vendor=claude phase=3 artifact_path_spec='`artifact_path: "$SPEC_PATH"`' artifact_revision_spec='`artifact_revision` (the current sha256 or git sha of the spec)' line_spec='`line`' evidence_spec='`evidence` (the exact excerpt)' -->
<!-- INCLUDE-END -->
...
1a. If `$VALIDATOR_ERRORS` is non-empty, fix every defect it names in the spec BEFORE touching your assigned findings, then re-run `validate_artifact spec-fixer "$DISPATCH_ID"` to confirm it now passes. A previous attempt already broke this artifact's structure; repeating its edits without repairing that first only reproduces the rejection.
...
6. Write the summary file at `$FEATURE_FOLDER/3-spec-review/spec-review-summary.md` with:
   ...
   - Residual MAJOR list — MAJOR findings still open in the final catalog at the passing iteration, whether or not they carry a `deferred:<followup_id>` / `accepted_risk:<decision_id>` disposition (spec §17.3). Non-empty ONLY when the gate passed under the relaxed rule (final iteration ≥ 3, `blockers=0`); empty for a strict pass (final iteration ≤ 2 with `majors=0`). For each, record its finding_id, source reviewer, location, disposition (or `none`), and one-line summary so the readiness writer can surface it. State explicitly whether the relaxed tier's single final fixer batch ran and whether it was re-reviewed: at iterations 3+ that final majors-only fix is authorized to close the gate WITHOUT a further reviewer round, so these majors may be unreviewed-after-fix — say which, rather than claiming re-review.
