...
4. Apply the iteration-dependent gate against the catalog counts — **iterations 1–2:** run this step when `blockers + majors > 0`, and always loop back to step 1 for a full re-review afterwards; **iterations 3 and up:** `blockers = 0` alone passes the gate, so run this step only when `blockers > 0` (full fix→re-review loop, as in iterations 1–2) or when majors remain and no final batch has run yet at this tier — in that one case dispatch the final batch below and then go straight to step 5 (gate passed), WITHOUT looping back to step 1:
   ...
   - **Iterations 1–2 only:** increment N and loop from step 1 — reviewers ALWAYS run again against the fixer's new commit in the strict tier. **Iterations 3 and up:** loop back to step 1 only if this round's trigger was a BLOCKER; if it was majors-only, this was the single authorized unreviewed final fix — record every still-open major in `followups.jsonl` via `append_followup`, then go to step 5 and pass the gate.
...
If iteration cap (`review_iteration_cap`) trips with any active reviewer still reporting an open BLOCKER, HALT. A cap reached with `blockers=0` is NOT a HALT however many majors remain — it passes per the relaxed gate (and sets the readiness verdict to `READY_WITH_NOTES`), with residual majors recorded in the gate summary and `followups.jsonl`.
...
- Optional inputs: `implementation_base_sha;run_log;relevant_artifacts;continuation_path;declared_foreign_changes;validator_errors;consolidation_priority`
...
- `$VALIDATOR_ERRORS` — mechanical output from a structural validator (`validate_artifact`) that REJECTED a previous attempt's edit (empty on a first attempt). Deterministic tool output, not review feedback: when non-empty, repairing exactly what it names is your FIRST duty, ahead of your assigned findings, and `DONE` is a lie until the named defect is gone.
...
1a. If `$VALIDATOR_ERRORS` is non-empty, repair every defect it names BEFORE touching your assigned findings, and re-run `validate_artifact implementation-fixer "$DISPATCH_ID"` to confirm it passes. A previous attempt broke this artifact's structure; repeating its edits without repairing that first only reproduces the rejection.
