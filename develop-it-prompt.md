...
file under `$PROCESS_REPO_ROOT/runtime/` — computed as the sha256 of the
concatenated per-file `sha256sum` lines, document first, runtime files in
`LC_ALL=C` sorted order. `develop_it_dirty` is one of four typed states
...
member differs, `untracked` when `git ls-files --error-unmatch` finds any
...
- `render_prompt APPENDIX_NAME` (or `render_prompt --check ROLE`) — extract one appendix and substitute orchestration variables via python3 (multi-line-safe, no `sed`); fails loudly on any unresolved `$VAR`.
...
- `appendix_exists ROLE` — true iff `<!-- BEGIN: ROLE -->` exists in `$PROCESS_PATH`.
...
5. **Gate 5 — runtime + registries.** Calls `bootstrap_runtime` (see "Runtime
   extraction contract" above) to materialize `$FEATURE_FOLDER/
